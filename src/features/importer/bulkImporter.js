...
export function buildImportedContent(type,rows,creator){
 ...
 if(type==="writing")return{key:"writing",value:{id:`writing-${id}`,title:`Imported writing ${new Date().toLocaleDateString()}`,visibility:"private",creator,minutes:Number(rows[0]?.time_minutes)||20,type:"Writing tasks",sections:rows.map(x=>({title:x.title,prompt:x.prompt,passage:x.prompt,instructions:x.instructions,directions:x.instructions,timeMinutes:Number(x.time_minutes)||20,minWords:Number(x.min_words)||0,maxWords:Number(x.max_words)||0,rubric:x.rubric,sampleAnswer:x.sample_answer,questions:[]})),itemCount:rows.length}};
