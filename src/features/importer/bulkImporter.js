...
const TYPES={
 ...
 reading:{label:"Reading",headers:["passage_id","title","time_minutes","passage","question","choice_a","choice_b","choice_c","choice_d","correct_answer","explanation"],required:["passage_id","time_minutes","passage","question","choice_a","choice_b","correct_answer"]},
 listening:{label:"Listening",headers:["audio_id","title","time_minutes","audio_filename","transcript","question","choice_a","choice_b","choice_c","choice_d","correct_answer","explanation"],required:["audio_id","time_minutes","transcript","question","choice_a","choice_b","correct_answer"]},
...
function fileStep(type,t){return `<div class="import-panel"><h3>นำเข้า ${t.label}</h3><p>ใช้ไฟล์ CSV UTF-8 ที่มีชื่อคอลัมน์ตามแม่แบบ</p><div class="import-template-row"><span>ยังไม่มีไฟล์ตามรูปแบบ?</span><button class="btn" type="button" data-action="import-download-template" data-type="${type}">ดาวน์โหลดเทมเพลต CSV</button></div><label class="import-drop" data-import-drop><strong>ลากไฟล์ CSV มาวางที่นี่</strong><span>หรือคลิกเพื่อเลือกไฟล์</span><input id="bulkImportFile" type="file" accept=".csv,text/csv"><small>ระบบจะแสดงตัวอย่างให้ตรวจ และยังไม่บันทึกทันที</small></label><div id="importFileError" class="import-error hidden"></div></div>`}
