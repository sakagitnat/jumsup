...
function floatingTimer(item){const seconds=Math.max(60,Number(item?.minutes||10)*60),endsAt=Number(item?._endsAt||0);return `<div class="floating-exam-timer" aria-live="polite"><small>เวลาคงเหลือ</small><strong id="examTimer" data-seconds="${seconds}" data-ends-at="${endsAt}">${String(Math.floor(seconds/60)).padStart(2,"0")}:00</strong></div>`}

function question(q,index=0,prefix="q"){
 const id=q.id||`${prefix}-${index+1}`;
 return `<div class="question" id="${escapeHtml(id)}"><div class="question-kicker">ITEM ${index+1}</div><h3>${escapeHtml(q.prompt)}</h3>${(q.choices||[]).map((c,i)=>`<button class="choice" data-question="${escapeHtml(id)}" data-answer="${i}" aria-pressed="false"><span>${i+1}</span>${escapeHtml(c)}</button>`).join("")}</div>`;
}
...
export function renderList(kind,items){
 ...
 <div class="grid g2">${list.map(x=>`<div class="card practice-set-card"><div class="card-head"><span class="tag ${x.official||x.visibility==="public"?"green":"blue"}">${x.official?"ชุดทางการ":x.visibility==="public"?"สาธารณะ":"ส่วนตัว"}</span><span class="practice-kind">${escapeHtml(x.category||x.type||titles[kind])}</span></div><h3>${escapeHtml(x.title)}</h3><p>สร้างโดย @${escapeHtml(x.creator||"Jumsup")}${x.minutes?` · ${x.minutes} นาที`:""}${x.itemCount?` · ${x.itemCount} ข้อ`:""}</p><div class="actions"><button class="btn btn-primary" data-action="open-practice" data-kind="${kind}" data-id="${x.id}">เริ่มฝึก</button>${x.official?"":`<button class="btn" data-action="edit-practice" data-kind="${kind}" data-id="${x.id}">แก้ไข</button><button class="btn btn-danger" data-action="delete-practice" data-kind="${kind}" data-id="${x.id}">ลบ</button>`}</div></div>`).join("")||`<div class="empty">ยังไม่มีชุดฝึกในหมวดนี้</div>`}</div>`;
}
...
export function renderReading(item){
 ...
 return `${floatingTimer(item)}${Header("SECTION II","Reading Skill","โหมดอ่านแบบข้อสอบ + Readlang: แตะคำอังกฤษเพื่อดูคำแปลและเพิ่มเข้า Flashcard",`${item.itemCount||qs.length} items`)}
...
export function renderListening(item){
 ...
 return `${floatingTimer(item)}${Header("SECTION I","Listening & Speaking","Short / Long Conversation พร้อม TTS สำเนียงและความเร็วเสียง",`${item.itemCount||qs.length} items`)}
...
export function renderWriting(item){
 ...
 return `${floatingTimer(item)}${Header("SECTION III","Writing Skill","Text Completion, Grammar in Context และ Paragraph Organization",`${item.itemCount||qs.length} items`)}
...
export function renderMock(item){
 ...
 return `${floatingTimer(item)}${Header("FULL SIMULATION",item.title,"แบบจำลองโครงสร้าง 80 ข้อ 90 นาที พร้อมกระดาษคำตอบ",`${item.minutes||90} minutes`)}
