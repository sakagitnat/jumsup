...
export function renderProfile(s){
 ...
 <div class="card"><div class="skill-card"><div class="skill-icon">★</div><div class="grow"><b>${pro?"Jumsup Pro":"Jumsup Free"}</b><small>สิทธิ์ปัจจุบัน: ${proSource(s)}${profile?.pro_bonus_until?` · โบนัสถึง ${new Date(profile.pro_bonus_until).toLocaleDateString()}`:""}</small></div>${pro?`<button class="btn" data-action="billing-portal">จัดการสมาชิก</button>`:`<button class="btn btn-primary" data-nav="pricing">ดูแพ็กเกจและราคา</button>`}</div>${(s.payments||[]).length||pro?`<div class="billing-support-row"><span>การยกเลิก การชำระเงิน หรือรายการที่ไม่ถูกต้อง</span><button class="billing-support-link" data-action="open-refund-request">ความช่วยเหลือด้านการชำระเงิน</button></div>`:""}${(s.refunds||[]).length?`<details class="billing-request-history"><summary>ประวัติคำขอ ${s.refunds.length} รายการ</summary><div class="refund-history">${s.refunds.slice(0,5).map(r=>`<div class="refund-row"><span><b>${r.status}</b><small>${new Date(r.requested_at).toLocaleDateString()} · ${escapeHtml(r.reason)}</small></span><span class="tag ${r.status==="refunded"?"green":r.status==="rejected"||r.status==="failed"?"pink":"blue"}">${r.status}</span></div>`).join("")}</div></details>`:""}</div>
