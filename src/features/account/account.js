...
export function renderAccount(s,active="profile"){
 ...
 return `${Header("ACCOUNT","บัญชีของฉัน","จัดการข้อมูล สมาชิก และการตั้งค่าจากที่เดียว","Account")}<nav class="account-tabs" aria-label="เมนูบัญชี">${tabs.map(([id,label,icon])=>`<button class="account-tab ${active===id?"active":""}" data-account-tab="${id}"><span>${icon}</span>${label}</button>`).join("")}</nav><div class="account-panel">${content}</div>`;
}
