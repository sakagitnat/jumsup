...
export function bindContentBuilder(root,b,hooks){
 ...
 const pdf=root.querySelector("#builderPdf");if(pdf)pdf.onchange=async()=>{const file=pdf.files?.[0];if(!file)return;const status=root.querySelector("#pdfStatus");status.textContent="กำลังอ่านข้อความจาก PDF…";status.classList.add("loading");try{const text=await readPdf(file);if(text.trim().length<30)throw new Error("ไฟล์นี้ไม่มีข้อความที่อ่านได้ อาจเป็น PDF สแกนซึ่งต้องใช้ OCR");b.pdfName=file.name;b.pdfText=text;if(b.kind==="vocab"){const words=parseWords(text);if(words.length)b.words=words;b.pdfStatus=words.length?`สร้างร่างคำศัพท์ ${words.length} คำจาก ${file.name}`:"อ่านไฟล์ได้ แต่ยังจับคู่คำศัพท์ไม่ได้ กรุณาเพิ่มคำด้วยตนเอง"}else{const questions=parseQuestions(text);b.content=text.slice(0,15000);if(questions.length)b.questions=questions;b.pdfStatus=questions.length?`สร้างร่างคำถาม ${questions.length} ข้อจาก ${file.name} · กรุณาตรวจคำตอบที่ถูกก่อนบันทึก`:`นำข้อความจาก ${file.name} แล้ว แต่ไม่พบรูปแบบข้อสอบ กรุณาเพิ่มคำถาม`}hooks.rerender()}catch(e){b.pdfStatus="อ่าน PDF ไม่สำเร็จ: "+(e.message||"ไฟล์อาจถูกล็อกหรือเป็นภาพสแกน");hooks.rerender()}};
}
