...
export async function createApp(root){
 ...
 function practiceSectionEditor(kind,section={},index=0){
  ...
  return `<section class="practice-section-editor" data-practice-section><div class="practice-builder-row-head"><div><span class="tag blue">SECTION <span data-section-number>${index+1}</span></span><strong data-section-heading>${escapeHtml(section.title||`ส่วนที่ ${index+1}`)}</strong></div><div><button type="button" class="mini-btn" data-practice-copy-section>คัดลอก Section</button><button type="button" class="mini-btn danger" data-practice-delete-section>ลบ</button></div></div><div class="modal-two"><label>ชื่อ Section<input data-section-title value="${escapeHtml(section.title||"")}" placeholder="เช่น General article 1"></label>${kind==="mock"?`<label>ประเภท<select data-section-type><option value="reading" ${section.type==="reading"?"selected":""}>Reading</option><option value="listening" ${section.type==="listening"?"selected":""}>Listening</option><option value="writing" ${section.type==="writing"?"selected":""}>Writing</option></select></label>`:`${partControl}<label>${kind==="listening"?"สถานการณ์":"คำแนะนำ"}<input data-section-description value="${escapeHtml(section.description||section.situation||section.directions||section.instructions||"")}" placeholder="ข้อความที่ผู้ทำแบบฝึกจะเห็น"></label>`}</div><label>${contentLabel}<textarea data-section-content rows="6" placeholder="วางหรือพิมพ์เนื้อหาสำหรับ Section นี้">${escapeHtml(content)}</textarea></label><div class="practice-question-list">${questions.map((q,i)=>practiceQuestionEditor(q,i)).join("")}</div><div class="practice-section-footer"><button type="button" class="btn practice-add-question" data-practice-add-question>+ เพิ่มคำถามใน Section นี้</button><button type="button" class="btn btn-primary" data-practice-add-section-after>+ เพิ่ม Section ถัดไป</button></div></section>`;
 }
 ...
 function readPracticeSection(row,kind){
  ...
  else if(kind==="listening"){section.script=content;section.situation=description;section.part=part||"short_conversation"}
 ...
 async function openWord(word,target){
  ...
  pop.querySelector(".btn-primary").onclick=()=>{const meaning=pop.querySelector("[data-word-meaning]").textContent.trim(),deckId=pop.querySelector("[data-word-deck]").value;store.update(state=>{let decks=[...state.decks],index=decks.findIndex(d=>d.id===deckId);if(index<0){decks.push({id:`deck-${crypto.randomUUID()}`,name:"Reading vocabulary",visibility:"private",creator:state.profile?.username||"guest",words:[]});index=decks.length-1}const deck={...decks[index],words:[...(decks[index].words||[])]};if(!deck.words.some(x=>x.w.toLowerCase()===word.toLowerCase()))deck.words.push({w:word,m:cleanFlashMeaning(meaning),e:"",stress:word,source:"reading"});decks[index]=deck;return {...state,decks}});root.querySelectorAll(`.read-word[data-word="${CSS.escape(word)}"]`).forEach(x=>x.classList.add("saved"));close()};
  if(local)return;
  if(!currentUser){pop.querySelector("[data-word-meaning]").textContent="เข้าสู่ระบบเพื่อแปลออนไลน์ หรือเพิ่มคำนี้ไว้แปลภายหลัง";return}
  try{const data=await api("/api/translate",{method:"POST",body:JSON.stringify({text:word,target:store.get().lang==="en"?"th":store.get().lang})});if(pop.isConnected)pop.querySelector("[data-word-meaning]").textContent=data.translation||"ยังไม่พบคำแปล"}catch{if(pop.isConnected)pop.querySelector("[data-word-meaning]").textContent="แปลไม่สำเร็จ แต่ยังเพิ่มคำไว้ใน Flashcard ได้"}
 }
