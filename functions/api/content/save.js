...
export async function onRequestPost({request,env}){
 try{
  ...
  const tag=v=>v?cleanText(v,{min:1,max:40,name:"tag"}):null;
  const exam=tag(b.exam),skill=tag(b.skill),level=tag(b.level);
  ...
  if(kind==="vocab"){
    const {error}=await sb.from("vocab_sets").upsert({id,user_id:user.id,name:title,visibility,exam,skill,level,updated_at:new Date().toISOString()},{onConflict:"id"});if(error)throw error;
  }else{
    ...
    const {error}=await sb.from("practice_sets").upsert({id,user_id:user.id,kind,title,visibility,payload,exam,skill,level,updated_at:new Date().toISOString()},{onConflict:"id"});if(error)throw error;
  }
