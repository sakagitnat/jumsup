...
export async function onRequestPost({request,env}){
 try{
  ...
  const liveSub=sub?.payment_account==="stripe_live_th";
  if(liveSub&&["active","trialing"].includes(sub.status))return json({error:"Subscription is already active"},409,noStore(cors));
  let customer=liveSub?sub?.stripe_customer_id:null;
  if(!customer){
   const c=await stripeRequest(env,"/customers",{method:"POST",params:{email:user.email,metadata:{user_id:user.id}}});
   customer=c.id;
   await sb.from("subscriptions").upsert({user_id:user.id,stripe_customer_id:customer,stripe_subscription_id:null,status:"incomplete",payment_provider:"stripe",payment_account:"stripe_live_th",billing_currency:billingCurrency},{onConflict:"user_id"});
  }
  const session=await stripeRequest(env,"/checkout/sessions",{method:"POST",idempotencyKey:crypto.randomUUID(),params:{
    ...
    metadata:{user_id:user.id,plan,billing_currency:billingCurrency,payment_account:"stripe_live_th"},
    subscription_data:{metadata:{user_id:user.id,plan,billing_currency:billingCurrency,payment_account:"stripe_live_th"}}
  }});
